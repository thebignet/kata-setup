...
class ExampleSpec extends FlatSpec with Matchers {
  ...
  def getHighcard(cards: List[Card]) = cards.maxBy(_.value).value

  it should "get high card" in {
    getHighcard(List(King(), Ace())) should be(Ace().value)
    getHighcard(List(Ace(), King())) should be(Ace().value)
  }
  ...
  def getWinner(card1Player1: Card, card2Player1: Card, card1Player2: Card, card2Player2: Card) =
    ...
    else if(getHighcard(List(card1Player1, card2Player1)) > getHighcard(List(card1Player2, card2Player2))) Player1
    else Player2
